...
module Agentkit
  module RAG
    ...
    class Indexer
      def initialize(corpus_name: "default_corpus", store: nil, config: nil)
      ...
      attr_reader :corpus_name, :config, :store, :chunker
      ...
      def index_corpus(source, chunk_documents: true)
        ...
        store.insert_chunks(corpus_name, chunks, embeddings: embeddings)
      ...
      def index_slice(slice)
        ...
        store.insert_chunks(slice_corpus, chunks, embeddings: embeddings)
        {
      ...
      def generate_embeddings(texts)
        ...
        texts.map do |txt|
          ...
          vec || Array.new(1536) { rand } # fallback deterministic/test vector if fake
        end
